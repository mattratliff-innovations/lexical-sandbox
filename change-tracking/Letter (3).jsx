...
useEffect(() => {
// Check for changes on an interval to detect additions/deletions/edits/reordering
...
console.log('Change detection state changed:', { from: lastHasChangesRef.current, to: hasChanges });
lastHasChangesRef.current = hasChanges;
setHasUnsavedChanges(hasChanges);
}
} catch (error) {
// Silently handle any errors from checkForChanges
...
}, []); // Empty dependency - always active
...
alert(currentChanges.hasChanges ? `Letter has unsaved changes!\n\n${details.join('\n')}` : 'No unsaved changes detected.');
},
