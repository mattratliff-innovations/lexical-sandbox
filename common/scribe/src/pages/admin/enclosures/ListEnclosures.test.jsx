...
const setMockDataAndRenderComponent = async () => {
  mockAxios.onGet(`${APP_API_ENDPOINT}/enclosures`).reply(200, mockData);
  await renderComponent();
  await waitForLoadingToFinish();
};

const setErrorMockAndRenderComponent = async () => {
  mockAxios.onGet(`${APP_API_ENDPOINT}/enclosures`).timeout();
...
describe('ListEnclosures', () => {
  ...
  it('displays an error toast on error', async () => {
    ...
    const expected = 'There was an error retrieving the Enclosures list';
    const alert = await screen.findByRole('alert');
    expect(alert).toHaveTextContent(expected);
  });

  it('shows a message when there are no records', async () => {
    mockAxios.onGet(`${APP_API_ENDPOINT}/enclosures`).reply(200, []);
    await renderComponent();
    await waitForLoadingToFinish();
...
describe('ListEnclosures Druid Table', () => {
  beforeEach(() => {
    mockAxios.reset();
    jest.clearAllMocks();
  });
...
describe('Edit Enclosure', () => {
  it('Navigates to the edit page', async () => {
    ...
    expect(mockedUseNavigate).toHaveBeenCalledWith(`/admin/enclosures/${mockData[0].id}`);
  });

  it('Name column links to the edit page', async () => {
    await waitFor(() => setMockDataAndRenderComponent(mockData));

    expect(screen.getByTestId('edit-Enclosure Name A1')).toHaveAttribute('href', `/admin/enclosures/${mockData[0].id}`);
  });
});
