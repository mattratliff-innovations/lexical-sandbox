...
export default function ListHeadersTypes() {
  ...
  return (
    <>
      ...
      <DrTable className="h1size" title="Headers" headers={headers} data={headersList} defaultSortCol="name" sortDirection="asc" {...TableConfigs}>
        {headersList.map((rowdata) => (
          <React.Fragment key={`key-${rowdata.id}`}>
            ...
            <DrColumn name="actions" uniqueId={rowdata.id}>
              <DrButton
                ...
                id={`edit-${rowdata.id}`}
                onClick={() => redirect(`/admin/headers/${rowdata.id}`)}>
      ...
      <div className="row">
        <div className="col-2">
          <Link to="/admin/headers/create" className="btn btn-primary w-100">
