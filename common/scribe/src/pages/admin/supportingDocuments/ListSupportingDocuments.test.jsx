...
const mockData = [
  {
    ...
    name: 'Supporting Document Name A1',
  ...
  },
  {
    ...
    name: 'Supporting Document Name B2',
  ...
  },
  {
    ...
    name: 'Supporting Document Name C3',
  ...
  },
  {
    ...
    name: 'Supporting Document Name D4',
    createdAt: '2024-06-23T14:16:19.020Z',
  ...
  },
  {
    ...
    name: 'Supporting Document Name E5',
    createdAt: '2024-06-23T14:16:19.020Z',
...
const setMockDataAndRenderComponent = async () => {
  mockAxios.onGet(`${APP_API_ENDPOINT}/supporting_documents`).reply(200, mockData);
  await renderComponent();
  await waitForLoadingToFinish();
};

const setErrorMockAndRenderComponent = async () => {
  mockAxios.onGet(`${APP_API_ENDPOINT}/supporting_documents`).timeout();
...
describe('ListSupportingDocuments', () => {
  ...
  it('displays an error toast on error', async () => {
    ...
    const expected = 'There was an error retrieving the Supporting Documents list';
    const alert = await screen.findByRole('alert');
    expect(alert).toHaveTextContent(expected);
  });

  it('shows a message when there are no records', async () => {
    mockAxios.onGet(`${APP_API_ENDPOINT}/supporting_documents`).reply(200, []);
    await renderComponent();
    await waitForLoadingToFinish();
...
describe('ListSupportingDocuments Druid Table', () => {
  beforeEach(() => {
    mockAxios.reset();
    jest.clearAllMocks();
  });
...
describe('Edit Supporting Document', () => {
  it('Navigates to the edit page', async () => {
    ...
    expect(mockedUseNavigate).toHaveBeenCalledWith(`/admin/supportingdocuments/${mockData[0].id}`);
  });

  it('Name column links to the edit page', async () => {
    await waitFor(() => setMockDataAndRenderComponent(mockData));

    expect(screen.getByTestId('edit-Supporting Document Name A1')).toHaveAttribute('href', `/admin/supportingdocuments/${mockData[0].id}`);
  });
});
