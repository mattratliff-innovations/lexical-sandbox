...
export default function ToolbarPlugin({
  ...
  const reflectOlDomStyle = (editorLex, listNode, typeKey) => {
    const el = editorLex.getElementByKey(listNode.getKey());
    ...
    if (!typeKey) {
      ...
      el.setAttribute('type', OL_TYPE_MAP[typeKey] || '1'); // fallback to decimal
      el.style.setProperty('list-style-type', typeKey, 'important');
    }
  };
  ...
  const clearOrderedTypeOnNearestList = () => {
    editor.update(() => {
      ...
      if ($isListNode(node)) {
        node.setStyle(''); // persist on node (for export)
        reflectOlDomStyle(editor, node, '');
      }
    });
  };

  // Apply custom ordered list numbering style (force new list context; no DOM pokes beyond reflecting style)
  const applyOrderedListType = (typeKey /* '', 'lower-roman', 'upper-roman', 'lower-alpha', 'upper-alpha' */) => {
    // 1) If we're already in an ordered list, toggle it off first so the next OL starts fresh (no continuation)
    editor.update(() => {
      const selection = $getSelection();
      if (!$isRangeSelection(selection)) return;

      let n = selection.anchor.getNode();
      while (n && !$isListNode(n)) n = n.getParent?.();
      if ($isListNode(n) && n.getListType?.() === 'number') {
        editor.dispatchCommand(REMOVE_LIST_COMMAND, undefined);
      }
    });

    // 2) If caret is immediately after an ordered list, insert a separator paragraph
    editor.update(() => {
      const selection = $getSelection();
      if (!$isRangeSelection(selection)) return;
      ...
      const prev = anchorTop.getPreviousSibling?.();
      if (prev && $isListNode(prev) && prev.getListType?.() === 'number') {
        const sep = $createParagraphNode();
        sep.append($createTextNode('')); // visually empty barrier
        anchorTop.insertBefore(sep);
      }
    });

    // 3) Insert a fresh ordered list
    editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined);

    // 4) Persist the style on the new ListNode, then mirror it to the DOM on the next frame for immediate UI feedback
    editor.update(() => {
      const selection = $getSelection();
      if (!$isRangeSelection(selection)) return;
      ...
      if ($isListNode(node) && node.getListType?.() === 'number') {
        node.setStyle(typeKey ? `list-style-type: ${typeKey} !important` : '');

        requestAnimationFrame(() => {
          editor.update(
            () => {
              reflectOlDomStyle(editor, node, typeKey);
            },
            { discrete: true }
          );
        });
  ...
  const toolBarBtns = {
    ...
    fullscreen: {
      ...
      icon: isFullscreen ? <ArrowsCollapse /> : <ArrowsFullscreen />,
