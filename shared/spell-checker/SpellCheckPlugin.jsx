...
// =========================
// Ignore variables (including the brackets): typing and insertion aware
// =========================
function processIgnoredVariables(text) {
  if (!text) return [];
  const spans = [];
  let i = 0;
...
class LanguageToolService {
  ...
  processLanguageToolResponse(data, originalText) {
    ...
    const variableSpans = processIgnoredVariables(originalText);

    const ignoredSpans = [...customWordSpans, ...variableSpans];
      ...
      registerApplyHandler(editorId, (nodeKey, suggestion, originalText, issueType, action = 'apply') => {
        if (action === 'ignore') {
          editor.update(
            () => {
              const node = $getNodeByKey(nodeKey);
              if (node && $createSpellCheckNode(node)) {
                const textNode = $createTextNode(node.getTextContent());
                const word = node.getTextContent().toLocaleLowerCase();
          ...
          const timeoutDelay = navigator.userAgent.include('Chrome') ? 50 : 0;
          setTimeout(() => {
