...
jest.mock('../../../http/letters', () => ({
  returnLetterToDraftStatus: jest.fn(),
}));
...
describe('ScribeEditor Central Print Banner and Timer', () => {
  ...
  it('calls cancelCentralPrintJob when Cancel print job button is clicked', async () => {
    ...
    returnLetterToDraftStatus.mockResolvedValue({});
    ...
    expect(returnLetterToDraftStatus).toHaveBeenCalledWith('doc-1');
...
describe('Quick Actions Buttons', () => {
  it('shows ChangeSignatureButton under Quick Actions', async () => {
    render(<ScribeEditor {...mockProps} />);
    expect(screen.getByText(/Change Signature/i)).toBeInTheDocument();
  });

  it('does not shows ChangeSignatureButton under Quick Actions', async () => {
    ...
    render(<ScribeEditor {...propsWithFalse} />);
    expect(screen.queryByText(/Change Signature/i)).not.toBeInTheDocument();
  });
});
