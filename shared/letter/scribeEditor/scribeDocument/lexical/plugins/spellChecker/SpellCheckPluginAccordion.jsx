...
export default function SpellCheckPluginAccordion({ issues = [] }) {
  ...
  // Highlights corresponding spelling (word) or grammar issue in the editor area
  const toggleWordHighlight = (nodeKey) => {
    const word = document.querySelector(`[data-lexical-spell-check='${nodeKey}']`);
    const selected = !expandedSections[nodeKey];

    try {
      if (selected) {
        word.setAttribute('style', 'background-color: #FFE5E5;');
      } else {
        word.removeAttribute('style');
      }
    } catch (error) {
      throw new Error(`That word was not found in the editor: ${error}`);
    }
  };
  ...
  const toggleSection = (nodeKey) => {
    ...
    toggleWordHighlight(nodeKey);
  };

  // Collapse already expanded section(s) before expanding the next section
  const collapseAll = () => {
    const expandedSectionsNodeKeys = Object.keys(expandedSections);
    expandedSectionsNodeKeys.forEach((nodeKey) => {
      if (expandedSections[nodeKey]) {
        toggleSection(nodeKey);
      }
    });
  };

  const handleIssueTileClick = (nodeKey) => {
    collapseAll();
    toggleSection(nodeKey);
  };
          ...
          <ContentWrapper key={it.nodeKey} isExpanded={isOpen}>
            <SectionHeader>
              <SectionTitle
                ...
                onClick={() => {
                  if (!isOpen) {
                    handleIssueTileClick(it.nodeKey);
                  } else {
                    toggleSection(it.nodeKey);
                  }
                }}>
