import { LETTERHEADER_ADDRESS_BLOCK_SEARCH_TEXT } from '../../../ScribeDocumentConstants';
...
export const foreignAddressSuffix = (address, lineBreak) => {
  ...
  return `${city || ''} ${province || ''} ${postalCode || ''}${lineBreak}${country?.description || ''}`; // TODO: Temp fix, remove the || '' when country object is coming back from the api
};
...
export const invalidContact = (contact) => !contact || !contact.firstName;

export const resolveCareOf = (inCareOf, lineBreak) => {
  const inCareOfPrefix = 'C/O'.concat(' ');

  return inCareOf ? inCareOfPrefix + valueWithNewLineOrEmpty(inCareOf, lineBreak) : '';
};

const generateAddressSuffix = (foreignAddress, address, lineBreak) =>
  foreignAddress ? foreignAddressSuffix(address, lineBreak) : domesticAddressSuffix(address);

export const serializeAddress = (contact, lineBreak) => {
  if (invalidContact(contact)) {
    return LETTERHEADER_ADDRESS_BLOCK_SEARCH_TEXT;
  }
  ...
  return (
    `${firstName} ${middleName || ''} ${lastName}${lineBreak}${valueWithNewLineOrEmpty(firmName, lineBreak)}${resolveCareOf(inCareOf, lineBreak)}${valueWithNewLineOrEmpty(preAddress, lineBreak)}${street} ${aptSuiteFloor || ''}${lineBreak}` +
    `${generateAddressSuffix(foreignAddress, address, lineBreak)}`
  );
};

const contactToHtml = (contact) => serializeAddress(contact, '<br/>');

export const contactToText = (contact) => serializeAddress(contact, '\n');
