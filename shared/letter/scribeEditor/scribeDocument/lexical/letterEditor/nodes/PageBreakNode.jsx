...
import { createPagebreakBaseDom, getPageBreakFromDom, PAGEBREAK_KEY, PAGEBREAK_TYPE } from '../nodeDomSerializers/PageBreakSerializer';
...
class PageBreakNode extends DecoratorNode {
  ...
  showVariable() {
    if (this.__showPageBreak === false && this.__pagebreak === PAGEBREAK_SEARCH_TEXT) {
      return;
    }
  ...
  static createFromEditor(editorIsOpen) {
    return new PageBreakNode(PAGEBREAK_SEARCH_TEXT, !editorIsOpen);
  }

  static getPageBreakData(draft) {
    return draft?.sections?.find((section) => section.text.includes('data-page-break')) === undefined ? null : PAGEBREAK_KEY;
  }
