...
import { $createTextNode, $getRoot, $getSelection, $isRangeSelection, $isTextNode, COMMAND_PRIORITY_LOW, TextNode } from 'lexical';
...
window.endnoteManager = window.endnoteManager || {
  ...
  initializeFromLetter(letterData) {
    this.reset();
    ...
    if ($isTextNode(anchorNode)) {
      ...
      while ((match = endnoteRegex.exec(text)) !== null) {
        ...
        if (offset >= matchStart && offset <= matchEnd) {
          const [, endnoteText, footnoteId] = match;
        ...
        const processNode = (node) => {
          ...
          for (let i = 0; i < children.length; i++) {
            ...
            if ($isTextNode(child) && i + 1 < children.length) {
              ...
              const isSupElement = nextChild.getType && (nextChild.getType() === 'element' || nextChild.__element === 'sup');
