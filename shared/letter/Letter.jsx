...
import './Letter.css';
import { setHeaderData, sortSectionsByOrder, updateDraft } from './LetterUtil';
...
export default function Letter() {
  ...
  const renderToast = (typeFn, message) => {
    if (!typeFn && typeof typeFn !== 'function') return;
  ...
  const confirmDeleteLetter = () => {
    deleteLetter(uuid)
      ...
      .catch(() => renderToast(showToastError, 'Something went wrong, please try again.'));
  };
  ...
  const saveDraft = async (markAllClean) => {
    const savedDraft = await updateDraft(setDraft, letterEditorRef, defaultSignature, draft, markAllClean);
  ...
  const finalizeDraft = async (printType) => {
    const savedDraft = await updateDraft(setDraft, letterEditorRef, defaultSignature, draft);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        ...
        renderToast(showToastError, 'Something went wrong, please try again.');
      })
