  ...
  (
    {
      ...
      useSignature = false,
      changeTracking, // NEW: Add changeTracking prop
    },
    ref
  ) => {
    ...
    // NEW: Track section editors for change tracking
    const sectionEditorTracking = useRef({});
        ...
        <div>
          <LexicalEditor
            ...
            editorRefAssignmentFunction={(editor) => {
              ...
              // NEW: Register section editors with change tracker
              if (changeTracking && section.frontEndId && !section.locked) {
                // Store the editor reference for tracking setup
                sectionEditorTracking.current[frontEndId] = {
                  editor,
    ...
    useEffect(() => {
      const nextSections = draft.sections.map((section) => ({ ...section, frontEndId: section.id ?? uuidv4() })).toSorted((a, b) => a.order - b.order);

      setDraftState({ ...draft, ...{ sections: nextSections } });
      setOrganizationSignature(findOrganizationSignature(draft));
    }, [draft]);

    // NEW: Set up change tracking for section editors
    useEffect(() => {
      if (!changeTracking) return;

      // Set up tracking for each registered editor
      Object.entries(sectionEditorTracking.current).forEach(([frontEndId, data]) => {
        if (data.editor && !data.unregisterFn) {
          const { editor, sectionId } = data;

          // Register update listener directly on the editor
          const unregister = editor.registerUpdateListener(({ editorState, prevEditorState, tags }) => {
            ...
            if (currentContent !== prevContent) {
              changeTracking.handleSectionEditorDirty(sectionId, true);
            }
          });

          // Store the unregister function
          sectionEditorTracking.current[frontEndId].unregisterFn = unregister;

          // Register with the change tracker
          changeTracking.registerSectionEditor(sectionId, editor, {
            markClean: () => {
              changeTracking.handleSectionEditorDirty(sectionId, false);
            },
            markSpellCheckStart: () => {
              // Handled by tags in the listener
            },
            markSpellCheckEnd: () => {
              // Handled by tags in the listener
            },
      ...
      return () => {
        if (changeTracking) {
          Object.entries(sectionEditorTracking.current).forEach(([frontEndId, data]) => {
            // Unregister update listeners
            if (data.unregisterFn) {
              data.unregisterFn();
            }
            // Unregister from change tracker
            if (data.sectionId) {
              changeTracking.unregisterSectionEditor(data.sectionId);
            }
          });
        }
      };
    }, [changeTracking, draftState.sections]); // Re-run when sections change
        ...
        <div className="portraitUsLetter" ref={portraitUsLetterRef} data-testid="portraitUsLetter" id="portrait-us-letter">
          <div
            ...
            {draftState.startsWith && (
              <div className="editorContainer lock-container">{createEditor('starts-with-editor', draftState.startsWith, !draftState.startsWithLocked)}</div>
            )}
            ...
            {draftState.endsWith && (
              <div className="editorContainer lock-container">{createEditor('ends-with-editor', draftState.endsWith, !draftState.endsWithLocked)}</div>
            )}
...
ScribeDocument.propTypes = {
  ...
  // NEW: Add changeTracking prop type
  changeTracking: PropTypes.shape({
    registerSectionEditor: PropTypes.func,
    unregisterSectionEditor: PropTypes.func,
    handleSectionEditorDirty: PropTypes.func,
    dirtySections: PropTypes.arrayOf(PropTypes.string),
  }),
};
