class Api::Scribe::V1::OrganizationsController < ApplicationController
  ...
  def create
    ...
    if @organization.save
      @organization = Organization.includes(:letter_types, organization_address_xrefs: { address: [:state, :country] }, organization_header_letter_type_xrefs: [:letter_type, :header]).find(@organization.id)
  ...
  def show
    organization = Organization.includes(:letter_types, :organization_signatures, organization_address_xrefs: { address: [:state, :country] }, organization_header_letter_type_xrefs: [:letter_type, :header]).find(params[:id])
    render json: organization.as_xrefs_result
  end
