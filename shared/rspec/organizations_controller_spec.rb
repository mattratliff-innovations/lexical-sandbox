...
#   let(:organization) { create(:organization, days_forward: 30) }
...
#           org_header_associations = Organization.find(response_json['id']).organization_header_xrefs
#           expect(org_header_associations.length).to eq(1)
#           expect(org_header_associations[0].header_id).to eq(header.id)
...
#       context 'with valid params' do
#         before do
#           # Ensure required AppSettings exist for exclusion scopes used by Organization
#           create(:app_setting, :excluded_organization_letter_categories)
#           create(:app_setting, :excluded_organization_scanned_digital_paper_letter_categories)
#         end

#         it 'returns organization json' do
#           put "/api/scribe/v1/organizations/#{organization.id}",
#           params: { organization: org_hash }, as: :json
...
#           expect(org_address['premium_processing']).to be(true)
#           expect(org_address['default']).to be(true)
#           org_header_associations = Organization.find(organization.id).organization_header_xrefs
#           expect(org_header_associations.length).to eq(1)
#           expect(org_header_associations[0].header_id).to eq(header.id)
#           expect(response_json['occ']).to be(true)
#         end
...
#           put "/api/scribe/v1/organizations/#{organization.id}",
#           params: { organization: org_inactive_default_address_hash }, as: :json
...
describe Api::Scribe::V1::OrganizationsController do
  ...
  describe 'authentication filters' do
    ...
    it_behaves_like 'auth_filter', verb: :get, path: -> { '/api/scribe/v1/organizations' }, allowed: %i[groupadmin templateadmin superadmin developer], forbidden: %i[viewonly creator creatorprinter supervisor]
    it_behaves_like 'auth_filter', verb: :get, path: -> { "/api/scribe/v1/organizations/#{organization.id}" }, allowed: %i[groupadmin templateadmin superadmin developer], forbidden: %i[viewonly creator creatorprinter supervisor]
    it_behaves_like 'auth_filter', verb: :post, path: -> { '/api/scribe/v1/organizations' }, params: -> { { organization: { name: 'x' } } }, allowed: %i[superadmin developer], forbidden: %i[viewonly creator creatorprinter supervisor groupadmin templateadmin]
    it_behaves_like 'auth_filter', verb: :put, path: -> { "/api/scribe/v1/organizations/#{organization.id}" }, params: -> { { organization: { id: organization.id, name: 'z' } } }, allowed: %i[superadmin developer], forbidden: %i[viewonly creator creatorprinter supervisor groupadmin templateadmin]
    it_behaves_like 'auth_filter', verb: :put, path: -> { "/api/scribe/v1/organizations/default_address/#{organization.id}" }, params: -> { { organization: { organization_address_xrefs_attributes: [] } } }, allowed: %i[superadmin developer], forbidden: %i[viewonly creator creatorprinter supervisor groupadmin templateadmin]
    it_behaves_like 'auth_filter', verb: :get, path: -> { api_scribe_v1_organizations_available_organizations_for_user_path(user_id: create(:user).id) }, allowed: %i[supervisor groupadmin templateadmin superadmin developer], forbidden: %i[viewonly creator creatorprinter]
  end
  ...
  describe 'show' do
    ...
    describe 'with superadmin permissions' do
      ...
      before do
        create(:app_setting, name: AppSetting::EXCLUDED_ORGANIZATION_SCANNED_DIGITAL_PAPER_LETTER_CATEGORIES, active: true)
        create(:app_setting_organization_letter_category_xref, app_setting: app_setting, organization: organization, letter_category: excluded_organization_letter_category)
        authenticate_to_superadmin
      end
  ...
  describe 'Default Address' do
    let!(:original_default) do
      AddressOrganizationType.create!(organization: organization, street: 'original default street', apt_suite_floor: 'apt 2B', city: 'Miami', state_id: state.id, zip_code: '30165', country: country, pre_address: 'National Center', nickname: 'NC')
    end
    let!(:default_address) do
      OrganizationAddressXref.where(address_id: original_default.id).update(premium_processing: true, active: true, default: true)
    end
    let!(:new_default) do
      AddressOrganizationType.create!(organization: organization, street: 'new default Street', apt_suite_floor: 'floor 7', city: 'San Diego', state_id: state.id, zip_code: '12345', country: country, pre_address: 'Org 2', nickname: 'O2')
    end
    let(:new_default_address) do
      OrganizationAddressXref.where(address_id: new_default.id).update(premium_processing: true, active: false, default: false)
    end
    let(:action) do
      put "/api/scribe/v1/organizations/default_address/#{organization.id}", params: { organization: { organization_address_xrefs_attributes: [{ id: organization.organization_address_xrefs[1].id, default: default_address[0].default, address_attributes: { id: new_default.id, nickname: 'test nickname' } }] } }, as: :json
    end
