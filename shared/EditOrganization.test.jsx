...
const headersData = [
  {
    ...
    row1_col1: '<p>[[[LETTERHEADER_LETTER_DATE]]]</p>',
    row1_col2: '<p>[[[CIS_ADDRESS]]]</p>',
    row2_col1: '<p>[[[LETTERHEADER_ADDRESS_BLOCK]]]</p>',
    row2_col2: '<p>[[[LETTERHEADER_CIS_SEAL]]]</p>',
...
const mockDefaultHeadersCall = () => mockAxios.onGet(`${APP_API_ENDPOINT}/headers`);
const headersDataWithoutXrefs = headersData.map(({ organization_header_xrefs: _, ...rest }) => rest);
const mockSuccessfulDefaultHeadersCall = () => mockDefaultHeadersCall().reply(200, headersDataWithoutXrefs);
...
describe('EditOrganization', () => {
  beforeEach(() => {
    setMockData();
    mockSuccessfulDefaultHeadersCall();
  ...
  it('displays a successful message', async () => {
    ...
    // await userInstance.click(within(comboBoxContainer).getByText('Header2'));
    ...
    // expect(putData.organization.header_id).toBe(UPDATED_HEADER_ID);
  });
...
describe('EditOrganization Validations', () => {
  ...
  it('displays error message for invalid header mapping name', async () => {
    ...
    await screen.findByTestId('headers');
    ...
    await screen.findByText('Associated Header is required!');
  });
...
describe('Add Address', () => {
  ...
  it('allows a new address to be added', async () => {
    ...
    expect(screen.getByText(/Default Address/i)).toBeInTheDocument();
  });
...
describe('Default Address', () => {
  ...
  it('saves default modal on yes', async () => {
    ...
    expect(screen.getByText(/33 St, Fl 7/i)).toBeInTheDocument();
...
describe('Default Signature', () => {
  ...
  it('closes modal on no', async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId('defaultModalBody')).not.toBeInTheDocument());
  });
  ...
  it('saves default modal on yes', async () => {
    ...
    expect(await screen.findByText(/Default Signature/i)).toBeInTheDocument();
  });
