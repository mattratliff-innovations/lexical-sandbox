...
import { TableNode, TableRowNode, TableCellNode } from '@lexical/table';
import { EditorRefPlugin } from '@lexical/react/LexicalEditorRefPlugin';
import { useFootnotePlugin, FootNoteNode } from './plugins/FootNotePlugin';

// Component to register the footnote plugin
function FootnotePluginWrapper() {
  useFootnotePlugin();
  return null;
}
...
function Editor({
    ...
    const [editorState, setEditorState] = useState({
      ...
      // Updated editor config to include FootNoteNode
  ...
  return (
    <LexicalComposer initialConfig={updatedEditorConfig}>
      <div className="editor-container">
        {editorRefAssignmentFunction && (
          <EditorRefPlugin editorRef={editorRefAssignmentFunction}/>
        )}
