...
a);return{element:b}}static importDOM(){return{code:a=>null!=a.textContent&&(/\r?\n/.test(a.textContent)||z(a,"BR"))?{conversion:B,priority:1}:null,div:()=>({conversion:aa,priority:1}),pre:()=>({conversion:B,priority:0}),table:a=>C(a)?{conversion:ba,priority:3}:null,td:a=>{let b=a.closest("table");return a.classList.contains("js-file-line")||b&&C(b)?{conversion:D,priority:3}:null},tr:a=>(a=a.closest("table"))&&C(a)?{conversion:D,priority:3}:null}}static importJSON(a){return E().updateFromJSON(a)}updateFromJSON(a){return super.updateFromJSON(a).setLanguage(a.language)}exportJSON(){return{...super.exportJSON(),
language:this.getLanguage()}}insertNewAfter(a,b=!0){var c=this.getChildren(),e=c.length;if(2<=e&&"\n"===c[e-1].getTextContent()&&"\n"===c[e-2].getTextContent()&&a.isCollapsed()&&a.anchor.key===this.__key&&a.anchor.offset===e)return c[e-1].remove(),c[e-2].remove(),a=m.$createParagraphNode(),this.insertAfter(a,b),a;let {anchor:f,focus:g}=a;b=(f.isBefore(g)?f:g).getNode();if(m.$isTextNode(b)){e=F(b);for(c=[];;)if(m.$isTabNode(e))c.push(m.$createTabNode()),e=e.getNextSibling();else if(G(e)){for(var h=
0,k=e.getTextContent(),l=e.getTextContentSize();h<l&&" "===k[h];)h++;0!==h&&c.push(H(" ".repeat(h)));if(h!==l)break;e=e.getNextSibling()}else break;e=b.splitText(f.offset)[0];h=0===f.offset?0:1;h=e.getIndexWithinParent()+h;k=b.getParentOrThrow();l=[m.$createLineBreakNode(),...c];k.splice(h,0,l);(c=c[c.length-1])?c.select():0===f.offset?e.selectPrevious():e.getNextSibling().selectNext(0,0)}I(b)&&({offset:a}=a.anchor,b.splice(a,0,[m.$createLineBreakNode()]),b.select(a+1,a+1));return null}canIndent(){return!1}collapseAtStart(){let a=
m.$createParagraphNode();this.getChildren().forEach(b=>a.append(b));this.replace(a);return!0}setLanguage(a){let b=this.getWritable();b.__language=a||void 0;b.__isSyntaxHighlightSupported=y(a);return b}getLanguage(){return this.getLatest().__language}getIsSyntaxHighlightSupported(){return this.getLatest().__isSyntaxHighlightSupported}}function E(a){return m.$applyNodeReplacement(new A(a))}function I(a){return a instanceof A}function B(a){a=a.getAttribute("data-language");return{node:E(a)}}
...
class N extends m.TextNode{constructor(a="",b,c){super(a,c);this.__highlightType=b}static getType(){return"code-highlight"}static clone(a){return new N(a.__text,a.__highlightType||void 0,a.__key)}getHighlightType(){return this.getLatest().__highlightType}setHighlightType(a){let b=this.getWritable();b.__highlightType=a||void 0;return b}canHaveFormat(){return!1}createDOM(a){let b=super.createDOM(a);a=O(a.theme,this.__highlightType);d.addClassNamesToElement(b,a);return b}updateDOM(a,b,c){let e=super.updateDOM(a,
b,c);a=O(c.theme,a.__highlightType);c=O(c.theme,this.__highlightType);a!==c&&(a&&d.removeClassNamesFromElement(b,a),c&&d.addClassNamesToElement(b,c));return e}static importJSON(a){return H().updateFromJSON(a)}updateFromJSON(a){return super.updateFromJSON(a).setHighlightType(a.highlightType)}exportJSON(){return{...super.exportJSON(),highlightType:this.getHighlightType()}}setFormat(){return this}isParentRequired(){return!0}createParentElementNode(){return E()}}
function O(a,b){return b&&a&&a.codeHighlight&&a.codeHighlight[b]}function H(a="",b){return m.$applyNodeReplacement(new N(a,b))}function G(a){return a instanceof N}function F(a){let b=a;for(;G(a)||m.$isTabNode(a);)b=a,a=a.getPreviousSibling();return b}function P(a){let b=a;for(;G(a)||m.$isTabNode(a);)b=a,a=a.getNextSibling();return b}let Q={defaultLanguage:"javascript",tokenize(a,b){return x.tokenize(a,x.languages[b||""]||x.languages[this.defaultLanguage])}};
